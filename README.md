...
Writes the entered data into sample.txt.
If the file already exists, the old content is overwritten.
✅ Read File
Opens and reads the contents of sample.txt.
Displays the content line by line on the console.
Handles errors if the file is not found.
✅ Modify File (Append)
Allows the user to enter additional text.
Appends the new data to the existing file content.
Ensures that previous data is not deleted.
✅ Exit
